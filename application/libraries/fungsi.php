...
Class fungsi {
    ...
    public $namahari = array('Minggu','Senin','Selasa','Rabu','Kamis','Jumat','Sabtu');
    ...
    function tglindonesia($tgl) {
        ...
        $bln = $this->bulan2(date('m', strtotime($tgl)));
    ...
    function tanggal_jurnal($in) { // yyyy-mm-dd
    ...
    function terbilang($x) {
        ...
        if (intval($x) < 12)
            return " " . $abil[intval($x)];
        elseif (intval($x) < 20)
            return $this->terbilang(intval($x) - 10) . " Belas ";
        elseif (intval($x) < 100)
            return $this->terbilang(intval($x) / 10) . " Puluh " . $this->terbilang(intval($x) % 10);
        elseif (intval($x) < 200)
            return " Seratus " . $this->terbilang(intval($x) - 100);
        elseif (intval($x) < 1000)
            return $this->terbilang(intval($x) / 100) . " Ratus " . $this->terbilang(intval($x) % 100);
        elseif (intval($x) < 2000)
            return " Seribu " . $this->terbilang(intval($x) - 1000);
        elseif (intval($x) < 1000000)
            return $this->terbilang(intval($x) / 1000) . " Ribu " . $this->terbilang(intval($x) % 1000);
        elseif (intval($x) < 1000000000)
            return $this->terbilang(intval($x) / 1000000) . " Juta " . $this->terbilang(intval($x) % 1000000);
    }
    ...
    function realPath($path) {
        ...
        $linux = (strtolower($ua) == 'linux') ? true : false;
        $win = (strtolower($ua) == 'win' || strtolower($ua) == 'winnt') ? true : false;
    ...
    function msgBox($input, $title = 'INFO',$goTo='') {
        $output = "<script> 
                jAlert(\"$input\",\"$title\");
                window.location = '$goTo';
            </script>";
